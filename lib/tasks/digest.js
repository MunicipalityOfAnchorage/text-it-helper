    ...
    while (results || next) {
      results.forEach((result) => {
        ...
        flowValueNames.forEach((fieldName) => {
          ...
          // // If field name isn't in the list of Airtable fields, ignore.
          if (!flowValueNames.includes(fieldName)) {
            return;
          }

          // If the field value is null, ignore (seems unlikely this would happen).
          if (!fieldValue) {
            return;
          }
...
module.exports.send = async () => {
  ...
  console.log(data.modifiedRecordsByAirtableId);
